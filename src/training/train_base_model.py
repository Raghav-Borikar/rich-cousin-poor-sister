...
def train_base_model(args, logger):
    ...
    model = EncoderDecoderModel(
        ...
        src_lang=args.src_lang,
        tgt_lang=args.tgt_lang,
        lora_rank=args.lora_rank,         # Add these to your args
    ...
    for epoch in range(args.epochs):
        ...
        for step, batch in enumerate(tqdm(train_dataloader, desc=f"Epoch {epoch+1} (Train)")):
            with autocast('cuda'):
                outputs = model(
            ...
            if (step + 1) % args.log_interval == 0:
                logger.info(f"Epoch {epoch+1}/{args.epochs}, Step {step+1}/{len(train_dataloader)}, Loss: {loss.item():.4f}")
       ...
       # Calculate BLEU every epoch
        ...
        checkpoint_manager.save_checkpoint(
            ...
            filename=f"lora_epoch_{epoch+1}.pt",
            save_adapter=True
        )
